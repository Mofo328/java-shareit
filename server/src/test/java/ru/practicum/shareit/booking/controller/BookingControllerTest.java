...
class BookingControllerTest {
    ...
    void setUp() {
        ...
        item = new ItemInfoDto(1L, "item created");

        booker = new UserInfoDto(1L);

        bookingInfoDto = new BookingInfoDto();
        bookingInfoDto.setId(1L);
        bookingInfoDto.setStart(start);
        bookingInfoDto.setEnd(end);
        bookingInfoDto.setItem(item);
        bookingInfoDto.setBooker(booker);
        bookingInfoDto.setStatus(BookingStatus.WAITING);
    }
    ...
    void getAll() throws Exception {
        when(bookingService.getByBooker(1L, State.ALL))
    ...
    void getByOwner() throws Exception {
        when(bookingService.getByOwner(1L, State.ALL))
                .thenReturn(List.of(bookingInfoDto));
